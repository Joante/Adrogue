...
public class ControladorConsultasTren extends HttpServlet {
	...
	protected void procesarPeticion(HttpServletRequest request , HttpServletResponse response)throws ServletException, IOException {
		...
		try {
			...
			movimientos = movimientoAbm.traerMovimientoTrasporteTarjeta(desde, hasta, 1, tarjeta.getNroTarjeta());  
			...
			request .getRequestDispatcher( "/ConsultasTren.jsp" ).forward( request ,response );
