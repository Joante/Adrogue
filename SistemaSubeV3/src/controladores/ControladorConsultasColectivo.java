...
public class ControladorConsultasColectivo extends HttpServlet {
	...
	protected void procesarPeticion(HttpServletRequest request , HttpServletResponse response)throws ServletException, IOException {
		...
		try {
			...
			if(linea==0) {
				movimientos = movimientoAbm.traerMovimientoTrasporteTarjeta(desde, hasta, 2, tarjeta.getNroTarjeta());  
			}
			else {
				movimientos = movimientoAbm.traerMovimientosLineaColectivoTarjeta(desde, hasta, 2, linea, tarjeta.getNroTarjeta());
			}
			...
			request .getRequestDispatcher( "/ConsultasColectivo.jsp" ).forward( request ,response );
